...
# Coding agent: "claude" (default) or "cursor"
# Override via RALPH_AGENT env (e.g. RALPH_AGENT=cursor)
agent: claude
...
When creating a workspace, Ralph already copies `.ralph/` and the agent config dir (`.claude/` for Claude, `.cursor/` for Cursor) automatically. Use `copy_to_worktree` for additional files the agent might need that aren't committed to git (e.g., `.env` files) or that live outside those directories.
...
The PRD (Product Requirements Document) is a JSON file that drives the execution loop. It is generated during the PRD creation session (launched by `ralph new`): type `/finish` when the plan is solid. The agent updates the PRD during `ralph run`.
